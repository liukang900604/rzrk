...
<title>系统设置 - Powered By UNICORN</title>
...
$().ready(function() {
    ...
    var $mobileToSmsWrap = $("#mobileToSmsWrap");
    var $smsHost = $("#smsHost");
    var $smsUsername = $("#smsUsername");
    var $smsPassword = $("#smsPassword");
    var $mobileToSms = $("#mobileToSms");
    ...
    $smsTest.click( function() {
        ...
        if ($this.val() == "短信测试") {
            $this.val("发送短信");
            $mobileToSmsWrap.show();
        } else {
            if ($.trim($mobileToSms.val()) == "") {
                $.dialog({type: "warn", content: "请输入收信人号码!", modal: true, autoCloseTime: 3000});
                return false;
            }
            if ($.trim($smsHost.val()) == "") {
                $.dialog({type: "warn", content: "请输入短信服务器地址!", modal: true, autoCloseTime: 3000});
                return false;
            }
            if ($.trim($smsUsername.val()) == "") {
                $.dialog({type: "warn", content: "请输入短信用户名!", modal: true, autoCloseTime: 3000});
                return false;
            }
            if ($.trim($smsPassword.val()) == "") {
                $.dialog({type: "warn", content: "请输入短信密码!", modal: true, autoCloseTime: 3000});
                return false;
            }


            $.ajax({
                url: "setting!ajaxSendSmsTest.action",
                data: {"mobileToSms": $mobileToSms.val(), "setting.smsHost": $smsHost.val(),"setting.smsUsername": $smsUsername.val(), "setting.smsPassword": $smsPassword.val()},
                type: "POST",
                dataType: "json",
                cache: false,
                beforeSend: function(data) {
                    $smsTestStatus.html('<span class="loadingIcon">&nbsp;</span>正在发送测试短信,请稍后...');
                    $this.attr("disabled", true);
                },
                success: function(data) {
                    $smsTestStatus.empty();
	...
	<div class="body">
		<form id="validateForm" class="form" action="setting!update.action" enctype="multipart/form-data" method="post">
			<ul id="tab" class="tab">
                ...
                <li>
                    <input type="button" value="短信设置" hidefocus />
                </li>
				<li>
			...
			<table class="inputTable tabContent">
				...
				<tr>
					...
					<td>
						<label>
							<@checkbox name="setting.isShowInventory" value="${setting.isShowInventory}" />前台是否显示商品库存
						</label>
					</td>
				</tr>
			</table>
            ...
            <table class="inputTable tabContent">
                <tr>
                    ...
                    <td>
                        <label>
                        <@checkbox name="setting.smsEnabled" value="${setting.smsEnabled}" />开通短信提醒
                        </label>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" id="smsHost" name="setting.smsHost" class="formText" value="${setting.smsHost}" />
                        <label class="requireField">*</label>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="text" id="smsUsername" name="setting.smsUsername" class="formText" value="${setting.smsUsername}" />
                        <label class="requireField">*</label>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <input type="password" id="smsPassword" name="setting.smsPassword" class="formText" title="smsPassword"  title="留空则不进行密码修改"/>
                        <label class="requireField">*</label>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
						<span id="mobileToSmsWrap" class="hidden">
							<div>接收者号码: </div>
							<input type="text" id="mobileToSms" name="mobileToSms" class="formText" />
						</span>
                        <input type="button" id="smsTest" class="formButton" value="短信测试" hidefocus />
