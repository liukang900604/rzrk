	...
	<div class="body">
		...
		<table>
			<tr class="title">
				...
				<td colspan="2">
					客户名称: ${tempOrder.shipName}<br />
					会员名称: ${(tempOrder.member.username)!"-"}
				</td>
			</tr>
			<tr class="separated">
				<th colspan="2">
					订单编号: ${tempOrder.orderSn}
				</th>
				<th colspan="2">
					订购日期: ${tempOrder.createDate?string("yyyy-MM-dd")}
				</th>
				<th colspan="2">
			...
			<#list tempOrder.orderItemSet as orderItem>
				<tr>
					<td>
						${orderItem_index + 1}
					</td>
			...
			<tr class="separated">
				<td colspan="3">
					订单附言: ${tempOrder.memo}<br />
				</td>
				<td colspan="3">
					商品总金额: ${tempOrder.totalProductPrice?string(currencyFormat)}<br />
					配送费用: ${tempOrder.deliveryFee?string(currencyFormat)}<br />
					支付手续费: ${tempOrder.paymentFee?string(currencyFormat)}<br />
					订单总金额: ${tempOrder.totalAmount?string(currencyFormat)}
				</td>
			</tr>
			<tr>
				<td colspan="6">
			...
			<tr class="separated">
				...
				<td colspan="3">
					收 货 人: ${tempOrder.shipName}<br />
					收货地址: ${tempOrder.shipArea.displayName}${tempOrder.shipAddress}<br />
					邮政编码: ${tempOrder.shipZipCode}<br />
					联系电话: ${tempOrder.shipPhone}<br />
					联系手机: ${tempOrder.shipMobile}<br />
					配送方式: ${tempOrder.deliveryTypeName}
				</td>
			</tr>
			<tr>
				<td>
