...
<title>添加/编辑商品 - Powered By UNICORN</title>
...
$().ready(function() {
	...
	var $goodsBrandLoadingIcon = $("#goodsBrandLoadingIcon");
	
	var $goodsIsPromotion = $("#goodsIsPromotion");
	var inital = $goodsIsPromotion.is(":checked");
	$goodsIsPromotion.click(function() {
		inital = this.checked;
		
		$("#generalPrice").attr("disabled", !inital);
		$("#generalPoint").attr("disabled", !inital);
		$("#brandPrice").attr("disabled", !inital);
		$("#brandPoint").attr("disabled", !inital);
	});
	...
	$specificationTab.click( function() {
		var $th = $productTitle.find("th");
		var $tr = $productTitle.parent().find("tr");
		var len = $th.length;
		if(!inital) {
			for(var i=len-5;i>len-9;i--) {
				$($th[i-1]).hide();
			}
			if($tr.length > 1) {
				for(var i=1;i<$tr.length;i++) {
					var $productTd = $($tr[i]).find("td");
					for(var j=len-5;j>len-9;j--) {
						$($productTd[j-1]).hide();
					}
				}
			}
	...
	$addProductButton.click( function() {
		...
		<@compress single_line = true>
			...
			productTrHtml += 
				'<td>
					<input type="text" name="productList[' + productIndex + '].generalPrice" class="formText productListGeneralPrice" style="width: 50px;" />
				</td>
				<td>
					<input type="text" name="productList[' + productIndex + '].generalPoint" class="formText productListGeneralPoint" style="width: 50px;" />
				</td>
				<td>
					<input type="text" name="productList[' + productIndex + '].brandPrice" class="formText productListBrandPrice" style="width: 50px;" />
				</td>
				<td>
					<input type="text" name="productList[' + productIndex + '].brandPoint" class="formText productListBrandPoint" style="width: 50px;" />
				</td>';
			}
			productTrHtml += 
				'<td>
	...
	$goodsCategoryId.change(function() {
		$.ajax({
			url: "goods!ajaxBrand.action",
			data: {id: $goodsCategoryId.val()},
			type: "POST",
			dataType: "json",
			cache: false,
			beforeSend: function() {
				$goodsBrandLoadingIcon.show();
				$("#brandId").empty();
			},
			success: function(data) {
				...
				if (data != null) {
					$.each(data, function(i) {
						<@compress single_line = true>
							optionHtml += 
							'<option value="' + data[i].id;
							if(data[i].id == '${(goods.brand.id)!}') {
								optionHtml += '" selected>';
							} else {
								optionHtml += '">';
							}
							optionHtml += data[i].name + 
							'</option>';
						</@compress>
					});
				}
	...
	$goodsForm.validate({
		...
		rules: {
			...
			"areaId": "required",
			"goods.goodsSn": {
				remote: "goods!checkGoodsSn.action<#if (goods.goodsSn)?? && !isCopyAction>?oldValue=${goods.goodsSn?url}</#if>"
			},
			"defaultProduct.productSn": {
				remote: "goods!checkProductSn.action<#if (defaultProduct.productSn)?? && !isCopyAction>?oldValue=${defaultProduct.productSn?url}</#if>"
			},
			"goods.score": {
				required: true,
				digits: true
			...
			"defaultProduct.generalPrice": {
				required: "#goodsIsPromotion:checked",
				digits: true
			},
			"defaultProduct.generalPoint": {
				required: "#goodsIsPromotion:checked",
				digits: true
			},
			"defaultProduct.brandPrice": {
				required: "#goodsIsPromotion:checked",
				digits: true
			},
			"defaultProduct.brandPoint": {
				required: "#goodsIsPromotion:checked",
				digits: true
			},
			"defaultProduct.weight": {
				required: true,
				digits: true
			},
		...
		messages: {
			...
			"defaultProduct.generalPrice": {
				required: "请填写通用积分价",
				digits: "通用积分价必须为零或正整数"
			},
			"defaultProduct.generalPoint": {
				required: "请填写所需通用积分",
				digits: "所需通用积分必须为零或正整数"
			},
			"defaultProduct.brandPrice": {
				required: "请填写品牌积分价",
				digits: "品牌积分价必须为零或正整数"
			},
			"defaultProduct.brandPoint": {
				required: "请填写所需品牌积分",
				digits: "所需品牌积分必须为零或正整数"
			},
			"defaultProduct.weight": {
				required: "请填写重量",
				digits: "重量必须为零或正整数"
...
<body class="input goods">
	<div class="bar">
		<#if isCopyAction>复制商品<#elseif isAddAction>添加商品<#else>编辑商品</#if>
	</div>
	...
	<div class="body">
		<form id="goodsForm" action="<#if isCopyAction>goods!paste.action<#elseif isAddAction>goods!save.action<#else>goods!update.action</#if>" method="post" enctype="multipart/form-data">
			...
			<table id="infoTable" class="inputTable tabContent">
				<tr>
					...
					<td>
						<select id="goodsCategoryId" name="goods.goodsCategory.id">
						...
						<span id="goodsBrandLoadingIcon" class="loadingIcon hidden">&nbsp;</span>
					</td>
				</tr>
				<tr>
					<th>
				...
				<tr>
					...
					<td>
						<label>
							<@checkbox id="goodsIsPromotion" name="goods.isPromotion" value="${(goods.isPromotion)!false}" />促销
						</label>
					</td>
				</tr>
				<tr class="specificationDisabledInfo">
					...
					<td>
						<input id="generalPrice" type="text" name="defaultProduct.generalPrice" class="formText" value="${(defaultProduct.generalPrice)!"0"}"<#if !((goods.isPromotion)!false)> disabled</#if> />
					</td>
				</tr>
				<tr class="specificationDisabledInfo">
					<th>
					...
					<td>
						<input id="generalPoint" type="text" name="defaultProduct.generalPoint" class="formText" value="${(defaultProduct.generalPoint)!}"<#if !((goods.isPromotion)!false)> disabled</#if> />
					</td>
				</tr>
				<tr class="specificationDisabledInfo">
					<th>
					...
					<td>
						<input id="brandPrice" type="text" name="defaultProduct.brandPrice" class="formText" value="${(defaultProduct.brandPrice)!"0"}"<#if !((goods.isPromotion)!false)> disabled</#if> />
					</td>
				</tr>
				<tr class="specificationDisabledInfo">
					<th>
					...
					<td>
						<input id="brandPoint" type="text" name="defaultProduct.brandPoint" class="formText" value="${(defaultProduct.brandPoint)!}"<#if !((goods.isPromotion)!false)> disabled</#if> />
					</td>
				</tr>
				<tr>
					<th>
					...
					<td>
						<input type="text" id="areaSelect" name="areaId" class="hidden" value="${(goods.area.id)!}" defaultSelectedPath="${(goods.area.path)!}" />
						<label class="requireField">*</label>
					</td>
				</tr>
				<tr class="specificationDisabledInfo">
				...
				<tr>
					...
					<td>
						<#assign tagSet = (goods.tagSet)! />
						<#list allTagList as tag>
						<label>
							<input type="checkbox" name="tagList.id" value="${tag.id}"<#if (tagSet.contains(tag))!> checked</#if> />${tag.name}
						</label>
						</#list>
					</td>
				</tr>
					...
					<#if (goods.isSpecificationEnabled)!>
						<#list (goods.productSet)! as product>
							<tr class="productTr" productIndex="${product_index}">
								...
								<td>
									<input type="text" name="productList[${product_index}].generalPrice" class="formText productListGeneralPrice" value="${product.generalPrice}" style="width: 50px;" />
								</td>
								<td>
									<input type="text" name="productList[${product_index}].generalPoint" class="formText productListGeneralPoint" value="${product.generalPoint}" style="width: 50px;" />
								</td>
								<td>
									<input type="text" name="productList[${product_index}].brandPrice" class="formText productListBrandPrice" value="${product.brandPrice}" style="width: 50px;" />
								</td>
								<td>
									<input type="text" name="productList[${product_index}].brandPoint" class="formText productListBrandPoint" value="${product.brandPoint}" style="width: 50px;" />
								</td>
								<td>
