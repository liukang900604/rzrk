...
<head lang="en">
    ...
    <script type="text/javascript" src="/rzrk/js/background/page_list.js"></script>
    ...
    <script type="text/javascript">
        window.isView = <#if isView?? && isView == 1>true<#else>false</#if>;
   		window.isEdit = <#if isEdit?? && isEdit == 1>true<#else>false</#if>;
   	    window.key = "${(key)!}"
   	    window.idItem = "${(idItem)!}"
		window.nameItem = "${(nameItem)!}"
	    window.page = "${(page)!}"
