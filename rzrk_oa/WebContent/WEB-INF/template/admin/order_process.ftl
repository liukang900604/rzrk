...
<title>处理订单 - Powered By UNICORN</title>
...
$().ready( function() {
	...
	var $splitProducts = $('#splitProducts');
	
	
	$splitButton.click(function(){	
		var $idsCheckedCheck = $("#splitOrderTable input[name='ids']:checked");
		if($idsCheckedCheck == null || $idsCheckedCheck == ""){
		    $.dialog({type: "success", content: "请选择需要拆分的订单项！", width: 380, modal: true, autoCloseTime: 3000});
		    return false;
		}
	
		$.dialog({type: "warn", content: "您确定要拆分吗?", ok: "确 定", cancel: "取 消", modal: true, okCallback: splitOrder});			
        function splitOrder (){
           		$.ajax({
					url : "/rzrk/admin/order!splitOrder.action?orderId=${order.id}",
					type : "POST",
					dataType : "json",
					...
					success : function(data) {
						if (data.status == "success") {
							$.dialog({type: "success", content: "订单拆分成功！", width: 380, modal: true, autoCloseTime: 3000});
							$idsCheckedCheck.parent().parent().remove();
						$idsCheckedCheck.attr("checked", false);
						$.message({type: data.status, content: data.message});
						}else if(data.status == "error"){
						   $.dialog({type: "success", content: data.message, width: 380, modal: true, autoCloseTime: 3000});
						}
			}
		});
        }
	...
	$splitProducts.click(function(){	
	var $idsCheckedCheck = $("#splitProductTable input[name='ids']:checked").serialize();
		if($idsCheckedCheck == null || $idsCheckedCheck == ""){
		    $.dialog({type: "success", content: "请选择需要拆分的订单项！", width: 380, modal: true, autoCloseTime: 3000});
		    return false;
		}
	
	var $splitProductQuantity = "";
	var $quantity = 0;
	<#list order.orderItemSet as orderItem>
	     $quantity = $('#${orderItem.id}').val();
	     if(null == $quantity || $quantity == ""){
	         $quantity = 0;
	     }
	     $splitProductQuantity += '&${orderItem.id}='+$quantity;	     
	</#list>
	$.dialog({type: "warn", content: "您确定要拆分商品吗?", ok: "确 定", cancel: "取 消", modal: true, okCallback: splitProduct});		
    function splitProduct (){
       		$.ajax({
				url : "/rzrk/admin/order!splitProducts.action?orderId=${order.id}"+$splitProductQuantity,
				type : "POST",
				dataType : "json",
				...
				success : function(data) {
					if (data.status == "success") {
						$.dialog({type: "success", content: "商品拆分成功！", width: 380, modal: true, autoCloseTime: 3000});
					}else if(data.status == "error"){
						   $.dialog({type: "success", content: data.message, width: 380, modal: true, autoCloseTime: 3000});
						}
		}
...
<body class="input">
	...
	<div class="body">
		<ul id="tab" class="tab">
			...
			<li>
				<input type="button" value="拆分订单"<#if order.orderStatus == "completed" || order.orderStatus == "invalid"> disabled</#if> hidefocus />
			</li>
			<li>
				<input type="button" value="拆分商品"<#if order.orderStatus == "completed" || order.orderStatus == "invalid"> disabled</#if> hidefocus />
			</li>
		</ul>
		...
		<div class="tabContent">		  		
			<table id="splitOrderTable" class="inputTable">
				<tr class="title">
					<th>货号</th>
					<th>商品名称</th>
					<th>价格</th>
				...
				<#list order.orderItemSet as orderItem>
					...
					<tr>
						<td>		
					  		<#if (order.orderItemSet.size()!=1)>
		                     <input type="button" id="splitButton" value="立即拆分"  />
		
		                    <#else>
		                       <input type="button" id="splitButton" value="${order.orderItemSet.size()}件商品，不能拆分"  disabled=true/>   
		...
		<div class="tabContent">
			<table id="splitProductTable" class="inputTable">
				<tr class="title">
					<th>货号</th>
					<th>商品名称</th>
					<th>价格</th>
				...
				<#list order.orderItemSet as orderItem>
				   <#if orderItem.productQuantity != 1>
					<tr>
						...
						<td>
							<input type="text" id="${orderItem.id}" value="0" />
						</td>
						<td>
							<input type="checkbox" name="ids" value="${orderItem.id}" />
						</td>
					...
					<tr>					
						<td>			                    
		                    <input type="button" id="splitProducts" value="拆分商品"  />                        
						</td>
					</tr>
			</table>
		</div>
