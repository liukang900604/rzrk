...
function initEvents(){
	...
	saveBtn.unbind('click').click(function(){
		...
		var len = rows.length;
		var workType = window.workType;
		if(workType == 1){
			for (var i = 0; i < len; i++){
				names.push(rows[i].编号);
		...
		if(workType == 1){//提交开发审批
			if(window.isEdit){//提交开发审批编辑过来的
				window.top.insertDateByWindow('内置流程编辑',ids.join(","),'#requestId');
				window.top.insertDateByWindow('内置流程编辑',names.join(","),'#requestName');
			}else{
				window.top.insertDateByTab('提交开发审批',ids.join(","),'#requestId');
				window.top.insertDateByTab('提交开发审批',names.join(","),'#requestName');
			}
		}
