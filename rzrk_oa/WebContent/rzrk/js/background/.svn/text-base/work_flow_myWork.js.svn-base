...
function operations(value, rowData, index) {
	...
	if(rowData.jsStatus == 4){
		    ...
		    if(rowData.internal == 2){
		    	link += "<a href='#' data-id='{0}' data-idx='{1}'  class='editInternal'>编辑</a>";
		    }else{
...
function checkOperations(value, rowData, index) {
	var link = "";
     link += "<a href='#' data-id='{0}' data-idx='{1}' class='view'>查看</a>";
    //return $.validator.format(link, rowData.productId, index, rowData.productName);
    return  $.format(link, rowData.id, index);
...
function getCheckColumns() {
    var columns = [[
            ...
            {title: "结束时间", field: "endDate", width: 120, align: 'center', sortable: true},
            {title: "最后审批人", field: "lastName", width: 120, align: 'center', sortable: true},
            {title: "操作", field: "operation", width: 80, align: 'center', formatter: checkOperations}
        ]];
    return columns;
}
...
function getConfig(){
    return {
        loadMsg: '正在获取数据......',
        url: '/admin/work!getAjaxWorkList.action?statu=1',
        method: 'get',
        width: '100%',
       // toolbar:getToolbar(),
        rownumbers: true,
...
function getCheckConfig(){
    return {
        loadMsg: '正在获取数据......',
        url: '/admin/work!getAjaxWorkList.action?statu=5',
        method: 'get',
        width: '100%',
        //toolbar:getToolbar(),
        rownumbers: true,
...
function getHandleConfig(){
    return {
        loadMsg: '正在获取数据......',
        url: '/admin/work!getAjaxWorkList.action?statu=2',
        method: 'get',
        width: '100%',
        //toolbar:getToolbar(),
        rownumbers: true,
