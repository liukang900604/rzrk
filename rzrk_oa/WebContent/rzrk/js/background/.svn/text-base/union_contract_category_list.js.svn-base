...
function operations(value, rowData, index) {
    var link = "<a href='#' data-id='{0}' data-idx='{1}' data-title_name='{2}' class='record_list'>记录列表</a>";
    link += "<em>|</em>";
...
function getColumns() {
    var columns = [[
            ...
            {title: "模板结构", field: "fields", width: 1000, align: 'left'},
...
function getConfig(){
    return {
        loadMsg: '正在获取数据......',
        url: '/admin/union_contract_category!getAjaxList.action',
        method: 'get',
        width: '100%',
        rownumbers: true,
...
/*	$("#uploadContractCategoryAndData").width($("#uploadContractCategoryAndDataBtn").width());
    ...
    $(".datagrid").on("click", ".record_list", function () {
    	var id = $(this).data("id");
        var url = "/admin/union_contract!list.action?unionContractCategoryId="+id;
    	...
    	var url = "/admin/union_contract_category!edit.action?id=";
    	var id = $(this).data("id");
    	...
    	var url = "/admin/union_contract_category!downloadTemp.action";
    ...
    $('#addContractCategory').unbind('click').click(function(){
    	var url = '/admin/union_contract_category!add.action';
        var ezWin = window.top.createWin({ title: "添加类型" });
    ...
    $('#delContractCategory').unbind('click').click(function(){
    	...
    	var url = "/admin/union_contract_category!delete.action";
    	var ids = [];
