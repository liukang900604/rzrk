(function($){
	...
	function initPos(pos_data){
		var opt='';
		if(pos_data!=null && pos_data.jobList.length>0){
			var typesLen=0;
			for(var i=0;i<pos_data.jobList.length;i++){
				opt+='<option value='+pos_data.jobList[i].id+'>'+pos_data.jobList[i].name+'</option>';
			}
		}
...
//		}	
	...
	function eventRight(){
		...
		opts.each(function(){
			...
			if(0==flag){
				optstr+='<option value='+$(this).val()+'>'+currTreeNode.text+"-"+$(this).text()+'</option>';
			}
		});
		if(optstr!=''){
	...
	$.fn.selectDeputypos = function(options) {
        if(options=="show"){
        	alert("show");
