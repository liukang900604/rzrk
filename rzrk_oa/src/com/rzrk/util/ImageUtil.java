...
public class ImageUtil {
	...
	public static String getFormatName(File imageFile) {
		...
		try { 
			...
			if (!iterator.hasNext()) {
				String extname = FilenameUtils.getExtension(imageFile.getName());
				return extname;
			}
	 ...
	 * @return 图片文件路径    
	 */
	public static String copyCommonFile(ServletContext servletContext, File srcFile,String fileName) {
		if (srcFile == null) {
			return null;
		}
		...
		String destFilePath = SettingUtil.getSetting().getFileUploadRealPath() + "/" + CommonUtil.getUUID() + "." + formatName;
		File destFile = new File(servletContext.getRealPath(destFilePath));
     ...
     * @return 图片文件路径
     */
    public static String copyTempImageFile(ServletContext servletContext, File imageFile) {
        if (imageFile == null) {
            return null;
        }
        String formatName = getFormatName(imageFile);
        ...
        String destImagePath = SettingUtil.getSetting().getImageUploadRealPath()+ "/" + Math.abs(uuid.hashCode() % 16) +"/"+uuid + "." + formatName;
        File destImageFile = new File(servletContext.getRealPath(destImagePath));
        File destImageParentFile = destImageFile.getParentFile();
        if (!destImageParentFile.isDirectory()) {
            destImageParentFile.mkdirs();
