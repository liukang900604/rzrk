...
public class FileAction extends BaseAdminAction {
	...
	private static final String[] imageFormatName = new String[] {"jpg", "jpeg", "gif", "bmp", "png","pdf"};// 图片格式名称

	private Boolean isFileAction = true;// 是否为FileAction
	private File imgFile;// 上传图片文件
	private String imgFileFileName;// 上传图片文件名称
	private String path;// 浏览路径
	private String order;// 排序方式
	private String dir;//查看类型
	...
	public String ajaxUpload() throws Exception {
		...
		if(StringUtils.equalsIgnoreCase(dir, "image")){
			String uploadPath = ImageUtil.copyImageFile(getServletContext(), imgFile);
			jsonMap.put("error", 0);
			jsonMap.put("url", getContextPath() + uploadPath);
		}else{
			String uploadPath = ImageUtil.copyCommonFile(getServletContext(), imgFile, imgFileFileName);
			jsonMap.put("error", 0);
			jsonMap.put("url", getContextPath() + uploadPath);
		}
		return ajax(jsonMap);
	...
	public String ajaxBrowser() throws Exception {
		...
		if(StringUtils.equalsIgnoreCase(dir, "image")){
			rootPath = getRealPath(setting.getImageBrowsePath()) + "/";
			currentPath = rootPath + "/" + path;
			currentUrl = getContextPath() + setting.getImageBrowsePath() + "/" + path;
		}else{
			rootPath = getRealPath(setting.getFileBrowsePath()) + "/";
			currentPath = rootPath + "/" + path;
			currentUrl = getContextPath() + setting.getFileBrowsePath() + "/" + path;
		}
